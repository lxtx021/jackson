...
//        SwingUtilities
        ...
        server.bind(new InetSocketAddress("127.0.0.1",28888));
        server.register()
    }*/
    ...
    public static void main(String[] args) {
        try {
            ...
            System.out.println("server.status "+server.getLocalAddress());
            while (true){
                server.accept();
                int key = select.select();
                 Set<SelectionKey> sk = select.selectedKeys();
                for (SelectionKey s:sk
                     ) {
                    System.out.println(s.isAcceptable());
                }
            }
        } catch (IOException e) {
            e.printStackTrace();
